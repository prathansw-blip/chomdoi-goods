...
ผลตรวจล่าสุดก่อนอัปเดตเอกสารนี้ (24 กันยายน 2026): Emulator tests **30/30 ผ่าน** จาก 6 ไฟล์; `npm run build` ผ่านและมีคำเตือน JavaScript chunk เกิน 500 kB; UI จำลองล็อกอิน admin/cashier เปิดกะ ขาย เติมของ และเห็นยอดขายกับสต็อกอัปเดตในแท็บที่สอง ไม่มีการใช้ Firestore Production ในการทดสอบเหล่านี้ ยังไม่ได้ทดสอบ UI ครบทุกเส้นทาง เช่น offline/reconnect, เปลี่ยนสิทธิ์, settings และ stock ทุกกรณี

## งานที่ยังต้องทำก่อนขึ้น Production

1. ทดสอบ UI ที่ยังค้างด้วย Auth/Firestore Emulator รวมทั้งกรณีเขียนผิดพลาด อินเทอร์เน็ตหลุด และสองเครื่องแก้ข้อมูลพร้อมกัน ตรวจผลใน Firestore Emulator ไม่ใช่เฉพาะข้อความแจ้งสำเร็จ
