...
`src/data/db.js` ใช้ Firebase config ของ `demo-chomdoi-tests` เฉพาะ Vite mode `emulator`; mode ปกติชี้ไปที่ `chomdoi-house` จึงใช้ `npm run dev:emulator` เมื่อต้องการลองเว็บโดยไม่เชื่อม Production อย่าใช้ `npm run dev` สำหรับการทดสอบที่ตั้งใจให้ปลอดจากข้อมูลจริง
...
ผลตรวจล่าสุด (26 กันยายน 2026): Emulator tests **32/32 ผ่าน** จาก 6 ไฟล์; `npm run build` ผ่านและมีคำเตือน JavaScript chunk เกิน 500 kB ไม่มีการใช้ Firestore Production ในการทดสอบเหล่านี้ UI จำลองเคยทดสอบล็อกอิน admin/cashier เปิดกะ ขาย เติมของ และรับข้อมูลในแท็บที่สองแล้ว รอบนี้ทดสอบเพิ่มการเปลี่ยนสิทธิ์และปิดบัญชีขณะใช้งาน, เพิ่ม/แก้/ลบสินค้าและหมวด, บันทึก Settings รวมถึง LINE token จำลอง, และการชนกันของการแก้ราคาสินค้า: ระบบปฏิเสธค่าที่ล้าสมัยและแสดงค่าล่าสุดหลังปิดฟอร์ม เมื่อหยุด Firestore Emulator ชั่วคราวระหว่างบันทึก เว็บยังไม่แสดงผลสำเร็จหรือราคาใหม่; เมื่อเปิด Emulator กลับ รายการจึงถูกยืนยัน และตรวจราคาในฐานข้อมูลจำลองแล้ว ไฟล์ Export ดาวน์โหลดจริง เปิด JSON ได้ และแก้ชื่อไฟล์ให้ใช้วันที่ท้องถิ่นแล้ว การทดสอบกรณีคำสั่งเขียนล้มเหลวแบบ `unavailable` ใช้การจำลองข้อผิดพลาด; ยังไม่ได้ยืนยันการตัดอินเทอร์เน็ตของเครื่องลูกข่ายจริงขณะที่เครื่องอื่นบันทึกข้อมูลต่อ

## งานที่ยังต้องทำก่อนขึ้น Production

1. ทดสอบการตัดอินเทอร์เน็ตของเครื่องลูกข่ายจริงและเชื่อมต่อกลับใน UI ขณะที่อีกเครื่องบันทึกข้อมูลต่อด้วยข้อมูลจำลอง พร้อมตรวจผลใน Firestore Emulator; เก็บรายการ UI เส้นทางอื่นที่ยังไม่ได้ทดสอบก่อน cutover
...
3. ตรวจทุกเครื่องที่เคยใช้ระบบเดิมว่ามีรายการใน browser cache ซึ่งยังไม่ถึง Firestore หรือไม่ แล้วกระทบยอดก่อนสลับแอป เพราะ client ใหม่ไม่โหลด cache ยอดขายเก่ามาใช้
4. สำรองข้อมูลจริงแบบเข้ารหัสอีกครั้งก่อน cutover และเก็บสำเนานอกเครื่อง พร้อมทดสอบกู้คืน สำเนาท้องถิ่นที่เคยตรวจอยู่ใต้ `/Users/keng/.codex/backups/chomdoi-house/`; ยังไม่ใช่ backup สุดท้าย ณ วัน deploy
