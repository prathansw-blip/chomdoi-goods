...
| --- | --- | --- |
| Login | พนักงานกรอก username/password ใน webapp; เว็บตรวจกับ `users` ใน `/stores/chomdoi_main`; Firestore Rules เดิมไม่บังคับ login | พนักงานยังกรอก username/password ใน webapp เดิม; Firebase Auth ตรวจรหัสผ่านเบื้องหลัง และต้องมี `/staff/{uid}` ที่ active; พนักงานไม่เข้า Firebase Console และเว็บไม่สร้างบัญชีขณะ login |
...
2. ผู้ดูแลระบบเตรียม Firebase Auth account และ `/staff/{uid}` สำหรับพนักงานที่ได้รับอนุมัติ ตรวจบัญชีเดิม/บัญชี Auth ที่จับคู่ไม่ได้ โดยไม่คัดลอกรหัสผ่านลงเอกสารใหม่ แล้วให้พนักงานทดลองล็อกอินผ่าน webapp เท่านั้น หน้า Settings รุ่นใหม่ไม่มีฟังก์ชันเพิ่มหรือเปลี่ยนรหัสผ่านพนักงาน
