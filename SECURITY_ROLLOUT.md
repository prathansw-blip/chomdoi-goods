...
1. Provision Firebase Auth accounts through an administrator-controlled process. Staff continue to sign in only through the Chomdoi Goods webapp with their username and password; they do not need Firebase Console access or permission to edit Firebase directly. A server backend is optional for provisioning later; no Cloud Functions or Blaze plan is required for the chosen LINE setup. The existing Apps Script proxy receives the token from the browser and does not protect it.
