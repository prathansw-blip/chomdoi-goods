...
- `tests/sale.transaction.test.js` verifies two simultaneous sales, sale/restock/cancellation concurrency, stale stock rejection, shift-close protection, settings merge, and daily-check conflict using synthetic data.
...
- The local UI smoke test signed in as admin and cashier, opened a shift, recorded sales and a restock, and confirmed a second tab received the updated sales and stock. Offline/reconnect, role changes, and every settings flow still need UI verification before cutover.
