...
Firebase Hosting and `firestore.secure.rules` were deployed on 26 September 2026 at approximately 10:43 Bangkok time from `codex/secure-sync-prep`, application commit `22370b5`. Production is `https://chomdoi-house.web.app`. The deployed client uses Firebase Auth, active staff records and operation-specific transactions; the store no longer contains `users`. The release history was subsequently integrated into `main` by fast-forward. The dependency patch from application commit `13c9112` was deployed to Hosting only on the same day at 15:00:17; the login logo fix from `41f6a03` was deployed to Hosting only at 15:17:33 Bangkok time. The login/auth flow, business data handling, data model and secure Rules are unchanged from the cutover commit; these patches require no database migration.
...
## Login logo correction (26 September 2026; deployed to Hosting only)

Before sign-in, the webapp cannot read the protected `settings.companyLogo` from Firestore. The login page used the generic hotel emoji until the user authenticated; after sign-in it could read the uploaded logo. Commit `41f6a03` adds `src/assets/chomdoi-house-logo.png`, based on the logo supplied by the owner, as the login page's local fallback. It continues to use `settings.companyLogo` when the setting is available. No login code, Settings values, Firestore data or Rules changed.

`npm run build` passed; the existing chunk over 500 kB warning remains. The Hosting-only deploy completed at `2026-09-26T08:17:33Z`, release `1790410653000000`, version `93c6d493717a0e91`. Public requests verified all six deployed files against the build hashes, confirmed the new JS references the logo PNG, and checked the SPA rewrite. The verifier did not run an authenticated browser or contact Firestore/Auth. No test was written or run for this small static asset/fallback change.
