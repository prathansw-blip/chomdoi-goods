...
import { getHotelSupplies, getSupplyChecks, getSupplyRestocks, addHotelSupply, updateHotelSupply, deleteHotelSupply, addSupplyCheck, addSupplyRestock, getUsers, subscribe, getSettings } from '../data/store.js';
...
function draw(container) {
  ...
  container.querySelectorAll('[data-view]').forEach(btn => {
    btn.onclick = () => { viewMode = btn.dataset.view; draw(container); };
  });
...
function drawCheck(el, supplies, checks, today) {
  ...
  const pastChecks = checks.filter(c => c.date < today).sort((a, b) => b.date.localeCompare(a.date));
    ...
    <div class="card" style="margin-bottom:1.25rem">
      ...
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${supplies.map(s => {
              const prevCount = prevCheck ? (prevCheck.items.find(i => i.supplyId === s.id)?.count ?? '—') : '—';
              const todayCount = todayCheck ? (todayCheck.items.find(i => i.supplyId === s.id)?.count ?? '') : '';
              const restockQty = restocksSincePrev.filter(r => r.supplyId === s.id).reduce((sum, r) => sum + r.qty, 0);
        ...
        <div style="margin-top:1rem;display:flex;gap:0.75rem;align-items:center">
          <div class="form-group" style="margin:0;flex:1;max-width:250px">
            <select class="form-select" id="check-user-select">
              ...
              ${getUsers().filter(u => u.active !== false && u.role !== 'admin').map(u => {
  ...
  if (saveBtn) {
    saveBtn.onclick = () => {
      ...
      if (!userSelect.value) { showToast('กรุณาเลือกผู้เช็ค', 'error'); return; }
      ...
      const prevCheck = checks.filter(c => c.date < today).sort((a, b) => b.date.localeCompare(a.date))[0];
      const restocks = getSupplyRestocks();
      const notifyItems = items.map(item => {
        const supply = supplies.find(s => s.id === item.supplyId);
        const prevCount = prevCheck ? (prevCheck.items.find(i => i.supplyId === item.supplyId)?.count ?? null) : null;
        const restockQty = prevCheck
          ? restocks.filter(r => r.supplyId === item.supplyId && r.date > prevCheck.date && r.date <= today).reduce((sum, r) => sum + r.qty, 0)
          : 0;
        let usage = null;
...
function drawRestock(el, supplies) {
  ...
  const restocks = getSupplyRestocks().sort((a, b) => b.timestamp.localeCompare(a.timestamp)).slice(0, 20);
  ...
  document.getElementById('btn-restock-supply').onclick = () => {
    ...
    if (!userId) { showToast('กรุณาเลือกผู้เติม', 'error'); return; }
    if (qty <= 0) { showToast('กรุณากรอกจำนวน', 'error'); return; }
              ...
              <table class="table" style="font-size:0.85rem">
                ...
                <tbody>
                  ${check.items.map(item => {
                    const supply = supplies.find(s => s.id === item.supplyId);
                    const prevCount = nextCheck ? (nextCheck.items.find(i => i.supplyId === item.supplyId)?.count ?? null) : null;
                    const restockQty = nextCheck
                      ? restocks.filter(r => r.supplyId === item.supplyId && r.date > nextCheck.date && r.date <= check.date).reduce((sum, r) => sum + r.qty, 0)
                      : 0;
                    let usage = '—';
                    if (prevCount !== null) usage = (prevCount + restockQty) - item.count;
                    return `<tr>
...
function drawManage(el, supplies, container) {
  ...
  const checks = getSupplyChecks();
  const latestCheck = [...checks].sort((a, b) => b.date.localeCompare(a.date))[0] || null;
  const restocks = getSupplyRestocks();
      ...
      <div class="table-wrap">
        <table class="table">
          ...
          <tbody>
            ${supplies.map(s => {
              const threshold = s.lowStockThreshold ?? 10;
              const { text: remainText, isLow } = getLatestRemaining(s.id, s.unit, threshold);
              return `<tr${isLow ? ' style="background:rgba(239,68,68,0.08)"' : ''}>
  ...
  document.getElementById('btn-add-supply').onclick = () => showSupplyModal(container);
...
function showSupplyModal(container, editId = null) {
  const existing = editId ? getHotelSupplies().find(s => s.id === editId) : null;
  ...
  overlay.querySelector('#ms-save').onclick = () => {
    ...
    const lowStockThreshold = +document.getElementById('ms-threshold').value || 10;
    ...
    if (existing) {
      ...
      addHotelSupply({ id: generateId('hsupply'), name, unit, icon, lowStockThreshold });
