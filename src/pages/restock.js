...
import { getProducts, addRestockLog, subscribe, getSettings, getUsers } from '../data/store.js';
import { generateId, formatDateTime, showToast, getStockStatus, getProductIcon } from '../utils/utils.js';
...
function draw(container) {
  ...
  const users = getUsers().filter(u => u.role !== 'admin' && u.active !== false);
  ...
  container.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;align-items:start">
      ...
      <div class="card">
        ...
        ${logs.length === 0 ? '<div class="empty-state"><div class="empty-state-icon">📋</div><div class="empty-state-text">ยังไม่มีประวัติการเติมของ</div></div>' : `
  ...
  document.getElementById('btn-restock').onclick = () => {
    ...
    if (qty <= 0) { showToast('กรุณาใส่จำนวน', 'warning'); return; }
    if (!userId) { showToast('กรุณาเลือกผู้เติม', 'warning'); return; }
    ...
    showToast(`เติม ${product?.name} จำนวน ${qty} ชิ้น โดย ${user?.displayName} สำเร็จ`, 'success');
    draw(container);
  };
}

export function destroyRestock() { if (unsub) { unsub(); unsub = null; } }
