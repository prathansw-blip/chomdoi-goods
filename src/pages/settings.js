...
import { getSettings, updateSettings, subscribe, getUsers, addUser, updateUser, deleteUser } from '../data/store.js';
import { showToast, formatHour } from '../utils/utils.js';
import { testConnection } from '../utils/lineNotify.js';
import { exportData, clearAllData, initFirebase, loadData, FIREBASE_CONFIG } from '../data/db.js';
...
// ═══ Icon Picker Data ═══
const iconGroups = [
  { label: '🧃 เครื่องดื่ม', icons: ['💧','🧃','🧉','🧋','☕','🍵','🧂','🍼','🥛','🍺','🥤','🍹','🧀','🥃'] },
  { label: '🍔 อาหาร', icons: ['🍚','🍜','🍝','🍔','🍕','🌮','🌯','🍳','🍲','🥩','🥚','🥪','🍣','🍱','🍞','🥨','🥙','🥗'] },
  { label: '🍪 ขนม & ของหวาน', icons: ['🍪','🍫','🍬','🍭','🍩','🍰','🎂','🍦','🍧','🧁','🥐','🍿','🥜','🍟','🥨'] },
  { label: '🍎 ผัก & ผลไม้', icons: ['🍎','🍊','🍌','🍓','🍇','🍉','🍍','🥝','🍑','🍒','🥭','🍋','🍐','🧭','🥑'] },
  { label: '🍺 แอลกอฮอล์', icons: ['🍺','🍻','🍷','🍸','🥃','🍶','🍹','🥂'] },
  { label: '🧼 ของใช้ & อุปโภค', icons: ['🧼','🧴','🧹','🧺','🧷','🧶','🧳','🧰','🛡️','🧤','👚','👕','👖','🩴','💼','🎒','🧲','☂️','💡','🔋','📱','💻'] },
  { label: '💊 สุขภาพ & ความงาม', icons: ['💊','🩹','🏭','🧴','🧵','🦋','🌻','🌺','🌹','🌾','✨','🌟'] },
  { label: '📦 ทั่วไป & อื่นๆ', icons: ['📦','🏷️','🛒','💰','💎','🏆','🎮','📚','✍️','📷','🎵','🎨','🏖️','⚽','🏀','🎯','🚗','✈️','🛕','🏨','🏠','🔑'] },
];
...
function showIconPicker(currentIcon, onSelect) {
  ...
  overlay.querySelectorAll('.icon-pick-btn').forEach(btn => {
    btn.onmouseenter = () => { if (!btn.classList.contains('selected')) btn.style.borderColor = 'var(--gold)'; };
    btn.onmouseleave = () => { if (!btn.classList.contains('selected')) btn.style.borderColor = 'var(--border)'; };
    btn.onclick = () => {
...
function draw(container) {
  ...
  container.innerHTML = `
    ...
    <div class="settings-section">
      ...
      <div class="theme-picker">
        ${[
          { id: 'dark-gold',   label: '🌑 Dark Gold',   colors: ['#0a0e1a','#1a2236','#f59e0b'] },
          { id: 'dark-blue',   label: '🌊 Dark Blue',   colors: ['#060d1f','#112240','#3b82f6'] },
          { id: 'dark-green',  label: '🌿 Dark Green',  colors: ['#061210','#122e29','#10b981'] },
          { id: 'dark-purple', label: '🌸 Dark Purple', colors: ['#0c0a1f','#1e1545','#8b5cf6'] },
          { id: 'light',       label: '☀️ Light',        colors: ['#f8fafc','#ffffff','#f59e0b'] },
        ].map(t => `
  ...
  container.querySelectorAll('.btn-pick-cat-icon').forEach(btn => {
    btn.onclick = () => {
      ...
      const hiddenInput = container.querySelector(`.s-cat-icon[data-idx="${idx}"]`);
  ...
  container.querySelectorAll('.btn-remove-cat').forEach(btn => {
    btn.onclick = (e) => {
      ...
      if (cats.length <= 1) { showToast('ต้องมีอย่างน้อย 1 หมวด', 'warning'); return; }
      // show confirm modal
      const overlay = document.createElement('div');
      overlay.className = 'modal-overlay';
      overlay.innerHTML = `
  ...
  logoInput.onchange = (e) => {
    ...
    if (!file) return;
    if (file.size > 500 * 1024) { showToast('ไฟล์ใหญ่เกินไป (สูงสุด 500KB)', 'error'); return; }
  ...
  if (removeBtn) removeBtn.onclick = () => { updateSettings({ companyLogo: null }); draw(container); };

  // LINE enabled toggle
  const lineToggle = document.getElementById('s-line-enabled');
  if (lineToggle) lineToggle.onchange = () => {
  ...
  // Export / Reset
  document.getElementById('btn-export').onclick = () => { exportData(); showToast('Export สำเร็จ', 'success'); };
...
function saveCurrentSettings(currentSettings) {
  ...
  const shiftDefs = (currentSettings.shiftDefinitions || []).map((def, i) => ({
    ...
    startHour: +(document.getElementById(`s-shift-start-${i}`)?.value ?? def.startHour),
    endHour: +(document.getElementById(`s-shift-end-${i}`)?.value ?? def.endHour),
  }));

  const updates = {
    companyName: document.getElementById('s-company').value.trim() || 'Chomdoi Goods',
...
export function destroySettings() { if (unsub) { unsub(); unsub = null; } }
...
function showUserModal(container, editId = null) {
  ...
  overlay.querySelector('#um-save').onclick = async () => {
    ...
    const username = overlay.querySelector('#um-username').value.trim().toLowerCase();
