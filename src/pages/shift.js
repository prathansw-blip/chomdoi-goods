...
import { getShifts, getTransactions, getProducts, getActiveShift, startShift, closeShift, deleteShift, subscribe, getSettings, getUsers } from '../data/store.js';
import { isAdmin, getCurrentUser } from '../utils/auth.js';
import { generateId, formatCurrency, formatDate, formatTime, formatDateTime, showToast, getBusinessDate, formatBusinessDate, getCurrentShiftDef, formatHour } from '../utils/utils.js';
import { sendShiftReport, sendDailySummary, sendShiftOpenNotification } from '../utils/lineNotify.js';
...
function paymentBreakdown(txns) {
  const cash = txns.filter(t => (t.paymentMethod || 'cash') === 'cash').reduce((s, t) => s + t.total, 0);
  const transfer = txns.filter(t => t.paymentMethod === 'transfer').reduce((s, t) => s + t.total, 0);
  const free = txns.filter(t => t.paymentMethod === 'free').reduce((s, t) => s + t.total, 0);
  return { cash, transfer, free, total: cash + transfer, grandTotal: cash + transfer + free };
}
...
function draw(container) {
  ...
  document.getElementById('btn-view-current').onclick = () => { viewMode = 'current'; draw(container); };
  document.getElementById('btn-view-history').onclick = () => { viewMode = 'history'; selectedDate = null; draw(container); };

  if (viewMode === 'current') drawCurrent(document.getElementById('shift-body'), settings, currency, active, currentDef, todayBiz);
...
function drawCurrent(el, settings, currency, active, currentDef, todayBiz) {
  ...
  let shiftTxns = [], shiftTotal = 0, shiftBills = 0, itemSales = {};
  if (active) {
  ...
  const topProduct = sortedItems[0] ? `${sortedItems[0][0]} (${sortedItems[0][1].qty} ชิ้น)` : '—';
  ...
  if (dailySummaryBtn) {
    dailySummaryBtn.onclick = async () => {
      ...
      const dayShiftsAll = getShifts().filter(s => (s.businessDate || getBusinessDate(s.startTime, startHour)) === todayBiz);
  ...
  el.querySelectorAll('.btn-start-shift').forEach(btn => {
    btn.onclick = () => {
      ...
      if (!def) return;
      const users = getUsers().filter(u => u.active !== false && u.role !== 'admin');
      ...
      overlay.querySelector('#open-confirm').onclick = () => {
        ...
        startShift({
          ...
          openedBy: opener ? { id: opener.id, displayName: opener.displayName } : null,
        });
        ...
        showToast(`เปิด ${def.icon} ${def.name} โดย ${opener?.displayName || 'Unknown'}`, 'success');
        overlay.remove();
  ...
  if (closeBtn) {
    closeBtn.onclick = () => {
      ...
      overlay.querySelector('#cs-confirm').onclick = () => {
        ...
        closeShift(active.id, closer ? { id: closer.id, displayName: closer.displayName } : null);
        const prods = getProducts();
        const lowStockItems = prods.filter(p => p.stock > 0 && p.stock <= (p.lowStockThreshold || 5));
        const shiftPbReport = paymentBreakdown(shiftTxns);
        sendShiftReport({
          name: active.name,
          date: formatBusinessDate(getBusinessDate(active.startTime, startHour)),
        ...
        if (lastShiftDef && active.defId === lastShiftDef.id) {
          ...
          const dayTxns = allTransactions.filter(t => (t.businessDate || getBusinessDate(t.timestamp, startHour)) === todayBizDate);
          const dayPb = paymentBreakdown(dayTxns);
          const dayShiftsAll = getShifts().filter(s => (s.businessDate || getBusinessDate(s.startTime, startHour)) === todayBizDate);
          sendDailySummary({
        ...
        showToast(`ปิด${active.name} โดย ${closer?.displayName || 'Unknown'}`, 'success');
        overlay.remove();
        draw(el.closest('.page-content'));
      };
    };
  ...
  el.querySelectorAll('.btn-view-bills').forEach(btn => {
    btn.onmouseenter = () => { btn.style.borderColor = 'var(--gold)'; btn.style.background = 'rgba(245,158,11,0.1)'; };
...
function showBillsModal(shiftId, shiftName, currency) {
  ...
  const payLabels = { cash: '💵 เงินสด', transfer: '📱 เงินโอน', free: '🎁 ฟรี' };
...
function drawHistory(el, settings, currency) {
    ...
    <div style="display:grid;grid-template-columns:280px 1fr;gap:1.25rem;align-items:start">
      ...
      <div class="card" style="max-height:600px;overflow-y:auto">
        ...
        ${sortedDates.map(date => {
          const dayTxns = transactions.filter(t => (t.businessDate || getBusinessDate(t.timestamp, startHour)) === date);
  ...
  el.querySelectorAll('.history-date-item').forEach(item => {
    item.onmouseenter = () => { if (item.dataset.date !== selectedDate) item.style.borderColor = 'var(--gold)'; };
    item.onmouseleave = () => { if (item.dataset.date !== selectedDate) item.style.borderColor = 'var(--border)'; };
    item.onclick = () => {
  ...
  if (selectedDate) {
    drawDayDetail(document.getElementById('history-detail'), selectedDate, settings, currency);
  }
}
...
function drawDayDetail(el, date, settings, currency) {
  ...
  const dayShifts = shifts.filter(s => (s.businessDate || getBusinessDate(s.startTime, startHour)) === date);
  const dayTxns = transactions.filter(t => (t.businessDate || getBusinessDate(t.timestamp, startHour)) === date);
  const dayTotal = paymentBreakdown(dayTxns).total;

  // Item breakdown
  const itemSales = {};
  dayTxns.forEach(t => t.items.forEach(item => {
    if (!itemSales[item.name]) itemSales[item.name] = { qty: 0, total: 0 };
    itemSales[item.name].qty += item.qty;
    itemSales[item.name].total += item.subtotal;
  }));
  ...
  el.innerHTML = `
    ...
    <div class="card" style="margin-bottom:1.25rem">
      ...
      ${dayShifts.map(s => {
        ...
        const sTotal = sPb.total;
        const sItems = {};
        sTxns.forEach(t => t.items.forEach(i => {
          if (!sItems[i.name]) sItems[i.name] = { qty: 0, total: 0, cash: 0, transfer: 0, free: 0 };
          sItems[i.name].qty += i.qty;
          sItems[i.name].total += i.subtotal;
          if (t.paymentMethod === 'cash') sItems[i.name].cash += i.subtotal;
          else if (t.paymentMethod === 'transfer') sItems[i.name].transfer += i.subtotal;
          else if (t.paymentMethod === 'free') sItems[i.name].free += i.subtotal;
        }));
        const sSorted = Object.entries(sItems).sort((a, b) => b[1].qty - a[1].qty);
        return `
          <div style="border:1px solid var(--border);border-radius:var(--radius-sm);padding:1rem;margin-bottom:0.75rem">
            ...
            ${sSorted.length > 0 ? `
              ...
              <table class="table" style="font-size:0.85rem">
                ...
                <tbody>${sSorted.map(([name, data]) => `<tr>
                  <td>${name}</td>
                  <td>${data.qty}</td>
...
function buildChart(txns) {
  ...
  if (!canvas) return;
  const hourlyData = {};
  txns.forEach(t => { const h = new Date(t.timestamp).getHours(); hourlyData[h] = (hourlyData[h] || 0) + t.total; });
  const hours = Object.keys(hourlyData).sort((a, b) => +a - +b);
  chartInstance = new Chart(canvas, {
    type: 'bar',
    data: {
      labels: hours.map(h => `${String(h).padStart(2, '0')}:00`),
      datasets: [{ label: 'ยอดขาย (฿)', data: hours.map(h => hourlyData[h]),
        backgroundColor: 'rgba(245,158,11,0.6)', borderColor: '#f59e0b', borderWidth: 1, borderRadius: 6 }]
    },
    options: {
      responsive: true,
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: ctx => `฿${ctx.raw.toLocaleString()}` }}},
      scales: { y: { beginAtZero: true, grid: { color: 'rgba(42,53,80,0.5)' }, ticks: { color: '#94a3b8' }}, x: { grid: { display: false }, ticks: { color: '#94a3b8' }}}
    }
  });
}
