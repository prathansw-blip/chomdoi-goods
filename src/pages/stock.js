...
import { getProducts, addProduct, updateProduct, deleteProduct, subscribe, getSettings } from '../data/store.js';
import { generateId, formatCurrency, showToast, getCategoryLabels, getCategoryList, getStockStatus } from '../utils/utils.js';
...
function draw(container) {
  ...
  const lowCount = products.filter(p => p.stock > 0 && p.stock <= (p.lowStockThreshold || 5)).length;
  ...
  // Events
  document.getElementById('stock-search').oninput = (e) => { searchTerm = e.target.value; draw(container); };
  document.getElementById('btn-add-product').onclick = () => showProductModal(container);
  container.querySelectorAll('[data-edit]').forEach(b => {
    b.onclick = (e) => { e.stopPropagation(); showProductModal(container, b.dataset.edit); };
  });
...
function showProductModal(container, editId = null) {
  ...
  photoInput.onchange = async (e) => {
    ...
    document.getElementById('m-photo-preview').innerHTML = '<span style="color:var(--text-muted);font-size:0.75rem">กำลังย่อรูป...</span>';
    const reader = new FileReader();
    reader.onload = async (ev) => {
      const compressed = await compressImage(ev.target.result, 150, 0.6);
      if (compressed) {
        pendingPhoto = compressed;
        document.getElementById('m-photo-preview').innerHTML = `<img src="${pendingPhoto}" style="width:100%;height:100%;object-fit:cover">`;
  ...
  if (removePhotoBtn) removePhotoBtn.onclick = () => {
    pendingPhoto = null;
    document.getElementById('m-photo-preview').innerHTML = '<span style="color:var(--text-muted);font-size:0.75rem">ไม่มีรูป</span>';
  };
  ...
  overlay.querySelector('#m-save').onclick = () => {
    ...
    if (!name || price <= 0) { showToast('กรุณากรอกข้อมูลให้ครบ', 'error'); return; }
    const data = {
      name, price,
    ...
    if (existing) { updateProduct(editId, data); showToast('แก้ไขสินค้าแล้ว', 'success'); }
    else { addProduct({ id: generateId('prod'), ...data }); showToast('เพิ่มสินค้าแล้ว', 'success'); }
    overlay.remove();
    draw(container);
  };
}

export function destroyStock() { if (unsub) { unsub(); unsub = null; } }
